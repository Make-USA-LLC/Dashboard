...
import { 
  Warehouse, User, Search, Package, History, 
...
const App = () => {
    // We trust auth.currentUser because the parent App enforces login
    const user = auth.currentUser;
    const [username, setUsername] = useState(user?.displayName || 'Staff');
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            if (editingItem) {
                ...
                await updateDoc(ref, { ...formData, updatedBy: username, updatedAt: serverTimestamp() });
                ...
                await addDoc(collection(db, COLLECTION_ROOT, 'public', 'items'), {
                    ...formData,
                    createdBy: username, createdAt: serverTimestamp(),
                    updatedBy: username, updatedAt: serverTimestamp()
                });
        ...
        <div className="flex h-screen w-full flex-col bg-slate-950 text-slate-100 font-sans">
                                ...
                                <div key={item.id} className="group relative flex items-center justify-between gap-4 rounded-xl border border-slate-800 bg-slate-900 p-4 shadow-sm transition-all hover:border-slate-700">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="flex items-center rounded-lg bg-slate-950 border border-slate-700">
                                            <button onClick={() => handleQuickUpdate(item, -1)} className="p-2 text-slate-400 hover:text-red-400 transition-colors" disabled={item.quantity <= 0}><Minus className="h-4 w-4" /></button>
                                            <span className="w-8 text-center text-sm font-bold text-white select-none">{item.quantity}</span>
                                            <button onClick={() => handleQuickUpdate(item, 1)} className="p-2 text-slate-400 hover:text-green-400 transition-colors"><Plus className="h-4 w-4" /></button>
                                        </div>
                                        <button onClick={() => openEditModal(item)} className="rounded-lg bg-slate-800 p-2 text-slate-300 hover:bg-orange-600 hover:text-white transition-colors"><Edit2 className="h-4 w-4" /></button>
                                    </div>
                                </div>
                            ))}
                        </div>
            ...
            <div className="fixed bottom-0 left-0 right-0 border-t border-slate-800 bg-slate-900 px-6 pb-6 pt-3 z-30 safe-bottom">
                <div className="flex items-center justify-around">
                    <button onClick={() => setCurrentView('inventory')} className={`flex flex-col items-center gap-1 ${currentView === 'inventory' ? 'text-orange-500' : 'text-slate-500'}`}><Package className="h-6 w-6" /><span className="text-xs font-medium">Inventory</span></button>
                    <button onClick={() => setCurrentView('logs')} className={`flex flex-col items-center gap-1 ${currentView === 'logs' ? 'text-orange-500' : 'text-slate-500'}`}><History className="h-6 w-6" /><span className="text-xs font-medium">History</span></button>
                </div>
            </div>
                        ...
                        <div className="p-6">
                            <form onSubmit={handleSubmit} className="space-y-4">
                                <div>
                                    ...
                                    <input type="text" required value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })} className="w-full rounded-lg border border-slate-700 bg-slate-800 p-3 text-white focus:border-orange-500 focus:outline-none" />
                                </div>
                                <div className="grid grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })} className="w-full rounded-lg border border-slate-700 bg-slate-800 p-3 text-white focus:border-orange-500 focus:outline-none" />
                                    </div>
                                </div>
                                <div>
                                    <label className="mb-1 block text-sm font-medium text-slate-400">Notes (Optional)</label>
                                    <textarea rows="2" value={formData.notes} onChange={(e) => setFormData({ ...formData, notes: e.target.value })} className="w-full rounded-lg border border-slate-700 bg-slate-800 p-3 text-white focus:border-orange-500 focus:outline-none" />
                                </div>
