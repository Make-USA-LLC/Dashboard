...
import { doc, onSnapshot, updateDoc, collection, addDoc, query, where, getDocs, getDoc } from 'firebase/firestore';
...
import { 
    ...
    Users, User, Mail, Phone, Trash2, Building2, ShieldCheck 
} from 'lucide-react';
...
export default function ClientDetail() {
    ...
    // --- MICROSOFT AUTH LOGIC (REDIRECT FLOW) ---
    ...
    const triggerUpload = async (type) => {
        // 🔒 BLOCK IN DEMO MODE
        if (isDemo) {
            alert("🔒 File uploads are disabled in the interactive demo.");
            return;
        }
    ...
    const handleDocumentUpload = async (e, type) => {
        // 🔒 BLOCK IN DEMO MODE
        if (isDemo) {
            alert("🔒 SharePoint integration is disabled in demo mode.");
            e.target.value = null;
            return;
        }
        ...
        try {
            ...
            if(url) {
                await updateDoc(doc(db, "clients", clientId), { [`files.${type}`]: url });
            }
        } catch(err) {
    ...
    const handleAddSample = async (e) => {
        ...
        let imageUrl = '';
        if (sampleImage) {
            // 🔒 BLOCK UPLOAD IN DEMO MODE BUT ALLOW RECORD CREATION
            if (isDemo) {
                alert("🔒 Photo uploads to SharePoint are disabled in demo mode. Sample will be logged without an image.");
                imageUrl = ''; // Ensure it stays empty
            } else {
                const token = await getMsToken();
                if (!token) return; 
                ...
                try { 
                    ...
                    return alert("Photo Upload Failed: " + err.message); 
                }
            }
        }
        
        const sampleData = { clientId, clientName: client.name, ...newSample, imageUrl };
                        ...
                        {perms?.manage_samples && (
                          <form onSubmit={handleAddSample} className="bg-white border border-slate-200 rounded-3xl p-6 mb-8 shadow-sm grid grid-cols-2 md:grid-cols-4 xl:grid-cols-7 gap-4 items-center">
                            ...
                            <button 
                                ...
                                className={`flex items-center justify-center gap-2 bg-slate-50 border border-dashed border-slate-300 rounded-xl text-xs font-bold text-slate-500 p-3 transition-colors ${sampleImage ? 'border-green-400 text-green-600 bg-green-50' : 'hover:border-blue-500 hover:text-blue-600'} ${isDemo ? 'cursor-not-allowed opacity-60' : 'cursor-pointer'}`}
                            >
                                <UploadCloud size={16}/> {sampleImage ? "Image OK" : "Photo"}
                            </button>
