...
import { db, auth, loadUserData, newIpadDefaults } from './firebase_config.jsx';
import { 
  doc, 
  collection, 
  onSnapshot, 
...
const Dashboard = () => {
  ...
  const handleCreateIpad = async () => {
    ...
    try {
      ...
      // Update: Navigate to new React Route with BASE
        ...
        <ul className="nav-list">
            ...
            {hasPerm('admin', 'view') && (
             ...
             {(hasPerm('admin', 'edit') || hasPerm('workers', 'edit')) && (
                <li className="nav-item" onClick={() => navigate(`${BASE}/StaffManagement`)}>
            ...
            {canViewFinance && (
                <>
                    ...
                    {(hasPerm('admin', 'edit') || hasPerm('queue', 'edit')) && (
                         <li className="nav-item" onClick={() => navigate(`${BASE}/manual_ingest`)}>
                    ...
                    {/* Finance Input - Linked to FinanceInput */}
                    {hasPerm('finance', 'view') && (
                        <li className="nav-item" onClick={() => navigate(`${BASE}/FinanceInput`)}>
                            <div className="nav-item-main"><span className="material-icons" style={{color:'#f1c40f'}}>monetization_on</span> Finance Input</div>
                        </li>
                    )}
                    ...
                    {(hasPerm('commissions', 'view') || hasPerm('finance', 'view')) && (
                        <>
                            ...
                            <li className="nav-item" onClick={() => navigate(`${BASE}/AgentReports`)}>
                                <div className="nav-item-main"><span className="material-icons" style={{color:'#e91e63'}}>summarize</span> Agent Reports</div>
                            </li>
                        </>
                    )}

                    {/* Finance Setup - Linked to FinanceSetup (Requires Edit Perms) */}
                    {hasPerm('finance', 'edit') && (
                        <li className="nav-item" onClick={() => navigate(`${BASE}/FinanceSetup`)}>
                            <div className="nav-item-main"><span className="material-icons" style={{color:'#3498db'}}>settings</span> Finance Setup</div>
                        </li>
                    )}
                </>
            )}
            ...
            {canViewQueue && (
                <>
                    ...
                    {hasPerm('queue', 'view') && (
                         <li className="nav-item" onClick={() => navigate(`${BASE}/UpcomingProjects`)}>
                     ...
                     {hasPerm('summary', 'view') && (
                         <li className="nav-item" onClick={() => navigate(`${BASE}/ProjectSummary`)}>
                            <div className="nav-item-main"><span className="material-icons" style={{color:'#8e44ad'}}>summarize</span> Production Summary</div>
                        </li>
                    )}
                    {hasPerm('queue', 'edit') && (
                            <li className="nav-item" onClick={() => navigate(`${BASE}/ProjectOptions`)}>
