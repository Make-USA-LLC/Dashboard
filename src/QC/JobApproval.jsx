...
const styles = {
    ...
    syncBtn: { background: '#f1f5f9', color: '#475569', border: '1px solid #cbd5e1', padding: '5px 10px', borderRadius: '4px', cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }
};
...
const JobApproval = () => {
    ...
    useEffect(() => {
        const runBackgroundSync = async () => {
            ...
            for (const job of jobs) {
                // Skip if no folder or if we already checked it recently
                if (!job.sharepointFolder || autoSyncedJobs.current.has(job.id)) continue;
                
                autoSyncedJobs.current.add(job.id); // Mark as checked

                try {
                    const response = await fetch(`https://graph.microsoft.com/v1.0/sites/${SITE_ID}/drive/root:${job.sharepointFolder}:/children`, {
                        headers: { "Authorization": `Bearer ${token}` }
                    });

                    if (response.ok) {
                        const data = await response.json();
                        const liveFiles = data.value || [];
                        
                        const newTechSheets = [];
                        const newQcPhotos = [];
                        
                        liveFiles.forEach(file => {
                            if (file.name.startsWith("QC_")) {
                                newQcPhotos.push({ name: file.name, url: file.webUrl });
                            } else {
                                newTechSheets.push({ name: file.name, url: file.webUrl });
                            }
                        });

                        // If Firebase has files that SharePoint doesn't, OVERWRITE Firebase to delete the dead links
                        const oldTechLen = job.techSheets ? job.techSheets.length : 0;
                        const oldQcLen = job.qcPhotos ? job.qcPhotos.length : 0;

                        if (newTechSheets.length !== oldTechLen || newQcPhotos.length !== oldQcLen) {
                            await updateDoc(doc(db, "production_pipeline", job.id), {
                                techSheets: newTechSheets,
                                qcPhotos: newQcPhotos,
                                qcPhotosUploaded: newQcPhotos.length > 0
                            });
                        }
                    }
                } catch (error) {
                    console.error("Silent sync failed for", job.id, error);
                }
            }
        };
    ...
    const syncSharePointFiles = async (job) => {
        ...
        try {
            ...
            if (!token) {
                setSyncingId(null);
                return alert("Please connect your SharePoint account first.");
            }
            ...
            if (!response.ok) throw new Error("Could not fetch folder contents. The folder may have been renamed or deleted.");
            const data = await response.json();
            ...
            console.error(error);
            alert("Error syncing files: " + error.message);
        } finally {
            setSyncingId(null);
        }
    };
