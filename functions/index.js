...
// ==================================================================
const admin = require("firebase-admin");
// Make sure admin.initializeApp() is at the top of your index.js file!

exports.generateGuestVoucher = functions.https.onCall(async (data, context) => {
    ...
    try {
        const api = axios.create({ baseURL: UNIFI_URL, httpsAgent: agent });
        const loginRes = await api.post("/api/login", { username: UNIFI_USER, password: UNIFI_PASS });
        const cookie = loginRes.headers["set-cookie"];

        // Generate 1 voucher, valid for 12 hours (720 mins)
        const cmdRes = await api.post(`/api/s/${SITE_ID}/cmd/hotspot`, {
            cmd: "create-voucher",
            expire: 720, 
            n: 1 
        }, { headers: { Cookie: cookie } });

        const createTime = cmdRes.data.data[0].create_time;
        const statRes = await api.get(`/api/s/${SITE_ID}/stat/voucher`, { headers: { Cookie: cookie } });
        const newVoucher = statRes.data.data.find(v => v.create_time === createTime);

        // Format code to look like 12345-67890 (if it's 10 digits)
        const rawCode = newVoucher.code;
        const formattedCode = rawCode.length === 10 ? `${rawCode.slice(0,5)}-${rawCode.slice(5)}` : rawCode;

        await api.post("/api/logout", {}, { headers: { Cookie: cookie } });

        // LOG TO FIRESTORE
        await admin.firestore().collection('guest_wifi_logs').add({
            firstName,
            lastName,
            email,
            code: formattedCode,
